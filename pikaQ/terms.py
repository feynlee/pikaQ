...
# %% auto 0
__all__ = ['CURRENT_ROW', 'Term', 'Value', 'NullValue', 'FieldBase', 'Field', 'Criteria', 'ArithmeticExpression', 'kwargs_func',
...
class Field(FieldBase):
    ...
    def execute(self, **kwargs):
        ...
        if self.alias:
            return f"{q}{execute(self.name, **kwargs)}{q} AS {q}{self.alias}{q}"
        else:
            return f"{q}{execute(self.name, **kwargs)}{q}"
...
class ArithmeticExpression(FieldBase):
    ...
    def __init__(self, this, op, other: Criteria=None) -> None:
        super().__init__()
        self.this, self.op, self.other = this, op, other
        self.get_sql = self.execute
